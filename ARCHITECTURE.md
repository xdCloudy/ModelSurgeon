...
## Conversational control plane (bounded v3.0 product release)

The repository exposes a bounded v3.0 conversational product journey over the structured ModelSurgeon APIs and CLI workflows. The v2.0 autonomous optimizer release boundary freezes the evidence, compatibility, and claim policy; the v2.1 intent-record, v2.2 provider, v2.4 clarification, v2.5 negotiation, v2.6 typed-tool, **closed v2.7 stateful-campaign**, **closed v2.8 evidence-grounding**, and **closed v2.9 security** boundaries are integrated and audited by the [v3.0 release manifest](docs/research/v3.0-product-release-v1.json). The product remains bounded: its tool boundary, state store, explanation projections and security gate are not a general agent runtime, alternate optimizer, hostile-process sandbox, or proof authority. See the [v3.0 product boundary](docs/release/v3.0-conversational-product-boundary.md), [packaging/setup guide](docs/release/v3.0-packaging-and-setup.md), and [release checklist](docs/release/v3.0-acceptance-security-reproducibility-review.md).
...
- The centralized conversational policy resolver orders hard constraints above validated specs, approvals, tool capabilities, evidence status, prompts, and provider text. Unknown or contradictory trusted states fail closed, and compiler, dispatcher, campaign, and explanation records retain the winning source and rejected alternatives. See [`docs/design/policy-precedence.md`](docs/design/policy-precedence.md).
...
The v2.1–v3.0 work is staged: v2.1 freezes the intent compiler contract; v2.2 adds replaceable providers; v2.3 adds the first chat vertical slice; v2.4 freezes bounded clarification; v2.5 closes explicit negotiation; v2.6 freezes the typed tool boundary; **v2.7 closes canonical campaign state and bounded recovery**; **v2.8 closes bounded evidence-grounded explanations**; **v2.9 closes the bounded approval/security gate** with the versioned [adversarial resistance corpus](docs/testing/adversarial-resistance.md); **v3.0 integrates the bounded product journey**. Direct CLI/Python callers bypass the conversational layer and remain supported. A release boundary does not imply live external evidence, optimizer proof, hostile-process containment, or measurements absent from a canonical report.
