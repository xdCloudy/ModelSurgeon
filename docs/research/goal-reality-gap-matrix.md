...
| Physical surgery and cumulative state | The first-party HF runtime executes a bounded cumulative MLP-channel sequence, publishes a self-contained child with model and tokenizer files at every accepted step, reloads and smoke-tests each child, physically evaluates each reloaded child before acceptance, rolls back a violating child, rediscoveries the component graph/candidate features from each accepted child, and rehydrates the selected child on process restart. | Partial | Use the rediscovered state to drive the next candidate decision within the same campaign, bind the full parent/child sequence into the public optimize artifact record, and connect mixed attention/layer/low-rank edit sequences to the first-party runtime. |
...
| Stateful evidence flywheel | The first-party HF search retains real pre-mutation feature records for every measured candidate, publishes revision-keyed partitions through the atomic feature cache, and records cache paths/checksums for both original-state search and accepted-child rediscovery. | Partial | Use state-conditioned candidates for continued search and add reusable post-mutation outcome ingestion without allowing stale or conflicting partitions. |
