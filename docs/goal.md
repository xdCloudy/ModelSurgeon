...
The bounded v3.0 product release integrates these layers without changing
...
- bounded tool calling over stable ModelSurgeon APIs, now frozen as the v2.6
  capability-scoped boundary with explicit unsupported cells and retained
  negative evidence;
...
- chat as the primary user experience in the verified v3.0 journey while preserving direct CLI/Python access;
...
| v2.4 | Handle only the clarification necessary for vague, incomplete or contradictory objectives. | v2.3 chat and v2.1 ambiguity contract. | **Closed:** deterministic clarification, fail-closed conflict handling, typed measurable-target questions, deterministic replay/provenance, explicit language/metric limits, and retained negative/inconclusive evidence in the [release boundary](release/v2.4-clarification-boundary.md). |
...
| v3.0 | **Release candidate prepared for #491 closure:** make conversation the primary UX while preserving direct CLI/Python automation and local-first operation. | v2.1-v2.9 exit criteria and v2.0 release evidence. | Integrated setup, diagnostics, campaign, explanation, approval, migration, packaging and end-to-end acceptance. The [release manifest](research/v3.0-product-release-v1.json) explicitly separates verified, experimental, unsupported, unknown and deferred v3.1 behavior. |
