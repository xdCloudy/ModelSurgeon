...
| `modelsurgeon.evaluation` | Typed benchmark/evaluation reports, frozen protocol and baseline evidence, quantization-order attribution, physical compression/quality-loss Pareto evidence, profile-bound kernel/offload microbenchmarks, structured artifact reconciliation, compatibility and transfer-suite evidence, provenance-complete benchmark cells, and bounded llama.cpp validation. |
...
| `modelsurgeon.surgeon` | Typed predictor bundles, training, calibration, ranking, versioned current-state embeddings, state-dependent predictor contracts, fixed-budget ranking-objective studies, bounded structural-model comparisons, and explicit lineage/compatibility decisions. |
