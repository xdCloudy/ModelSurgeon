...
| `modelsurgeon.optimization` | Deterministic optimize plans plus the explicit `execute_first_party_optimize` Python boundary, named hardware/quality profiles, approval points, cost estimates, source/artifact lineage, resumable execution, and explicit supported/unsupported/failed/unknown outcomes. |
