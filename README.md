...
pretrained SmolLM-135M, Qwen2.5-0.5B, and SmolLM2-135M CPU cells. The core
record includes two accepted rediscovered-state surgery stages on the latter
two; the attention-head and transformer-layer tranches deliberately retain
strict-gate failures without promoting artifacts. The signed Meta-Surgeon
pilot was physically compared on held-out Qwen and SmolLM2 candidates; the
expanded safety-classifier pilot matched no-guidance on SmolLM2 but selected
an ineligible Qwen candidate (held-out AUC 0.375), so physical measurements
...
256-channel children reduced storage but all violated the declared perplexity
...
- real HF physical mutation, repair, and quantization runs plus native-GGUF
  physical mutation and external-runtime validation runs; and
- revision-pinned first-party acceptance cells on tiny Llama/Mistral and
  ...
  outcomes; and
- a real conversational acceptance campaign using a local Qwen GGUF text
...
- an explicitly pinned `llama-cli` executable can also back the local GGUF
  chat interpreter when `llama-cpp-python` is not installed
...
over tensors, measurements, approvals, or artifacts. When the optional Python
GGUF binding is unavailable, pass an explicitly pinned `llama-cli` executable
with `--runtime-executable`. Preview a plan first:

```bash
uv run modelsurgeon chat ./models/text-model.gguf \
  --runtime-executable ./llama-cli \
...
quantization stages retain explicit `unsupported` records rather than being
presented as successful operations. Set `quantization.method` to
`dynamic_int8` for the bounded CPU-first HF quantization arm; its packed
weights are stored in a safe-tensors artifact with a first-party manifest,
then reloaded and physically evaluated before acceptance. Set `repair.method` to `lora` for the
bounded first-party LoRA arm or `distillation` for teacher-logit repair; both
use real calibration examples, resource telemetry, reload results, and a
physical accept/reject decision. The default `repair.method: none` keeps
...
```mermaid
flowchart LR
    INPUT_HF["HF / safetensors<br/>(first-party optimize path)"] --> GRAPH["Canonical graph<br/>identity · coupling · constraints"]
