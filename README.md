...
presented as successful operations. Set `repair.method` to `lora` to run the
bounded first-party LoRA arm; its real calibration loss, resource telemetry,
reload result, and accept/reject decision are retained, and the default
`repair.method: none` keeps no-repair as an explicit control.
