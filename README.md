...
| Conversational adversarial resistance | **v2.9 bounded resistance evidence** | Versioned deterministic provider/tool corpus with prompt-injection, smuggling, forged-evidence, secret, path, authority-conflict, malformed-result and model-description cases; failures retain severity, remediation state, provenance and budgets. Hostile-process containment remains unclaimed. See the [resistance corpus](docs/testing/adversarial-resistance.md) and [evidence manifest](docs/research/v2.9-adversarial-resistance-v1.json). |
