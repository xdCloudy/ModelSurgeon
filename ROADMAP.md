...
[GitHub milestone](https://github.com/xdCloudy/ModelSurgeon/milestone/26). **Closed by issue #462.** The bounded exit is recorded in the [v2.5 constraint-negotiation release boundary](docs/release/v2.5-constraint-negotiation-boundary.md) and [machine-readable release record](docs/research/v2.5-constraint-negotiation-release-v1.json), which reconcile #458–#461 with the v2.0 approval boundary: grounded non-destructive infeasibility, measured-only Pareto alternatives, immutable user-approved amendments, preserved direct-API objective history, deterministic replay, and retained negative/unsupported/prediction-only/inconclusive outcomes. Automatic relaxation, live model-quality evidence, and general language negotiation remain unclaimed.
...
## v2.9 — Approval Gates and Conversational Security Hardening (closed)

[GitHub milestone](https://github.com/xdCloudy/ModelSurgeon/milestone/30). **Closed by issue #485 at a bounded pre-product gate.** The versioned [v2.9 conversational security boundary](docs/release/v2.9-conversational-security-boundary.md) and [machine-readable release record](docs/research/v2.9-conversational-security-release-v1.json) reconcile scoped approvals and material diffs, the retained adversarial corpus, provider/tool trust zones, centralized fail-closed policy, clean-environment replay, direct API preservation, and explicit residual risks. The gate does not claim hostile-process containment, live-provider security or quality, universal prompt-injection resistance, distributed recovery, optimizer proof, or product readiness.

## v3.0 — Conversational ModelSurgeon Product Release (bounded release candidate)

[GitHub milestone](https://github.com/xdCloudy/ModelSurgeon/milestone/31). **Prepared for closure by issue #491 as a bounded release candidate.** Conversation is the primary supported UX for the verified journey while direct CLI/Python automation remains supported. The integrated local-first workflow covers setup, provider diagnostics, campaign management, grounded explanations, approvals, migration guarantees, packaging guidance and end-to-end security/reproducibility acceptance. `Surgeon Tensors/` remains the learned Meta-Surgeon, `Text LLM/` remains the replaceable conversational model, and `Models/` remains user/target model data. The [release manifest](docs/research/v3.0-product-release-v1.json) lists verified, experimental, unsupported, unknown and deferred v3.1 behavior. v3.1+ returns the main research emphasis to the learned Meta-Surgeon.
