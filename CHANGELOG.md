...
## Unreleased
  ...
  - Added a content-addressed, user-supplied JSONL code exact-match evaluator
    ...
    quality-retention gate as perplexity; native GGUF remains fail-closed, while
    conversational coding requests now carry an explicitly declared existing
    local benchmark through the exact spec and engine boundary.
