"""Distinct Q4_K tensor codec and whole-file recipe metadata handling."""
...
class Q4KRecipe(StrEnum):
    Q4_K_S = "Q4_K_S"
    Q4_K_M = "Q4_K_M"


@dataclass(frozen=True, slots=True)
class Q4KRecipeMetadata:
    recipe: Q4KRecipe
    general_file_type: int
    tensor_type: GGMLQuantizationType = GGMLQuantizationType.Q4_K


_FILE_TYPES = {14: Q4KRecipe.Q4_K_S, 15: Q4KRecipe.Q4_K_M}


def resolve_q4_k_recipe(metadata: Mapping[str, object]) -> Q4KRecipeMetadata:
    raw = metadata.get("general.file_type")
    if not isinstance(raw, int) or isinstance(raw, bool) or raw not in _FILE_TYPES:
        raise CodecContractError("general.file_type is not a supported Q4_K_S/Q4_K_M recipe")
    return Q4KRecipeMetadata(_FILE_TYPES[raw], raw)
...
def _pack_scales(scales: Sequence[int], minima: Sequence[int]) -> bytes:
    ...
    for index, (scale, minimum) in enumerate(zip(scales, minima, strict=True)):
        if index < 4:
            packed[index], packed[index + 4] = scale, minimum
        else:
...
class Q4_KCodec:
    ...
    layout: CodecLayout = QUANT_LAYOUTS[GGMLQuantizationType.Q4_K]

    def validate_blocks(self, source: memoryview, *, byte_order: ByteOrder) -> BlockValidation:
        del byte_order
        valid = source.ndim == 1 and source.contiguous and source.nbytes % 144 == 0
        return BlockValidation(
            valid,
            source.nbytes // 144 if valid else 0,
            "valid Q4_K super-blocks" if valid else "partial or non-contiguous Q4_K block",
        )
    ...
    def encoded_block_range(
        ...
        if (
            ...
            raise CodecContractError("Q4_K super-block range escapes encoded source")
        return source.cast("B")[block_offset * 144 : (block_offset + block_count) * 144]
    ...
    def _decode_block(self, block: memoryview, byte_order: ByteOrder) -> tuple[float, ...]:
        ...
        if not math.isfinite(delta) or not math.isfinite(minimum_delta):
            raise CodecContractError("Q4_K block scales must be finite")
        packed, quants = block[4:16], block[16:144]
        output: list[float] = []
        scale_index = 0
        for segment in range(4):
            ...
            base = segment * 32
            output.extend(
                delta * scale1 * (quants[base + lane] & 15) - minimum_delta * min1
                for lane in range(32)
            )
            output.extend(
                delta * scale2 * (quants[base + lane] >> 4) - minimum_delta * min2
                for lane in range(32)
            )
            scale_index += 2
        return tuple(output)
    ...
    def decode_blocks(
        ...
        *,
        ...
        validation.require_valid()
        view = source.cast("B")
        for offset in range(0, len(view), 144):
            destination.extend(self._decode_block(view[offset : offset + 144], byte_order))
    ...
    def _encode_block(self, values: Sequence[float], byte_order: ByteOrder) -> bytes:
        ...
        for start in range(0, 256, 32):
            ...
            local_scales.append((max(group) + minimum) / 15.0)
        ...
        for group_index in range(8):
            ...
            levels.extend(
                max(0, min(15, _nearest((value + offset) / step))) if step else 0
        ...
        for segment in range(4):
            ...
            for lane, (one, two) in enumerate(zip(first, second, strict=True)):
                quants[segment * 32 + lane] = one | (two << 4)
        return encoded_deltas + _pack_scales(scales, minima) + bytes(quants)
    ...
    def encode_blocks(
        ...
        *,
        ...
        if destination.readonly or not destination.contiguous or destination.nbytes != expected:
            raise CodecContractError("Q4_K destination must be writable and exact-sized")
        if not all(math.isfinite(value) for value in source):
            raise CodecContractError("Q4_K input values must be finite")
        view = destination.cast("B")
        for index, start in enumerate(range(0, len(source), 256)):
            view[index * 144 : (index + 1) * 144] = self._encode_block(
